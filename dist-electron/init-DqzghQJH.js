...
var a = class {
	...
	{
		regex: /\b(prime\s+minister|president|parliament|election|narendra\s+modi|bjp|congress|politician)\b/i,
		reason: "GENERAL_KNOWLEDGE",
		label: "Politics / Government"
	},
	{
		regex: /\b(capital\s+of\s+[a-z]+|largest\s+country|population\s+of\s+[a-z]+|who\s+is\s+[a-z\s]+(actor|singer|leader))\b/i,
		reason: "GENERAL_KNOWLEDGE",
		label: "General Geography / Facts"
	},
	{
		regex: /\b(quantum\s+physics|relativity|speed\s+of\s+light|gravity|black\s+hole|photosynthesis)\b/i,
		reason: "GENERAL_KNOWLEDGE",
		label: "Science / Physics"
	},
	{
		regex: /\b(cricket\s+score|ipl|world\s+cup|football|fifa|messi|ronaldo|olympics)\b/i,
		reason: "GENERAL_KNOWLEDGE",
		label: "Sports & Entertainment"
	},
	{
		regex: /^(hi|hello|hey|good\s+morning|good\s+afternoon|good\s+evening|howdy|sup)\b/i,
		reason: "CASUAL_CONVERSATION",
		label: "Casual Greeting"
	},
	{
		regex: /\b(how\s+are\s+you|how's\s+it\s+going|who\s+made\s+you|what('s|\s+is)\s+your\s+name|who\s+are\s+you)\b/i,
		reason: "CASUAL_CONVERSATION",
		label: "Conversational Banter"
	},
	{
		regex: /\b(tell\s+me\s+a\s+joke|make\s+me\s+laugh|funny\s+story|riddle)\b/i,
		reason: "CASUAL_CONVERSATION",
		label: "Entertainment / Humor"
	},
	{
		regex: /\b(what\s+should\s+i\s+(eat|cook|wear|buy|watch)|movie\s+recommendation)\b/i,
		reason: "PERSONAL_ADVICE",
		label: "Personal Advice"
	},
	{
		regex: /\b(how\s+to\s+lose\s+weight|workout\s+routine|diet\s+plan|headache\s+remedy)\b/i,
		reason: "PERSONAL_ADVICE",
		label: "Health / Lifestyle Advice"
	},
	{
		regex: /\b(should\s+i\s+buy\s+a\s+house|relationship\s+advice|career\s+advice)\b/i,
		reason: "PERSONAL_ADVICE",
		label: "Personal Consultation"
	},
	{
		regex: /\b(write(\s+me)?\s+(a\s+)?(poem|song|story|rap|birthday\s+message|essay|haiku))\b/i,
		reason: "CREATIVE_WRITING",
		label: "Creative Writing"
	},
	{
		regex: /\b(write|create|code|debug|explain)\s+(a\s+)?(python|javascript|typescript|java|c\+\+|golang|html|css)\s+(program|code|script|function)\b/i,
		reason: "PROGRAMMING_CODE",
		label: "Software Programming"
	},
	{
		regex: /\b(sort\s+an\s+array|reverse\s+a\s+linked\s+list|binary\s+tree|kubernetes|docker\s+container)\b/i,
		reason: "PROGRAMMING_CODE",
		label: "Computer Science / DevOps"
	},
	{
		regex: /\b(what\s+is\s+chatgpt|which\s+llm\s+is\s+better|how\s+does\s+gpt\s+work|what\s+is\s+artificial\s+intelligence)\b/i,
		reason: "GENERAL_AI",
		label: "General AI Questions"
	},
	{
		regex: /\b(stock\s+price\s+of\s+(apple|google|microsoft|tesla|amazon|meta|tcs|infosys))\b/i,
		reason: "UNRELATED_BUSINESS",
		label: "External Equities"
	},
	{
		regex: /\b(bitcoin|cryptocurrency|ethereum|solana|crypto\s+price)\b/i,
		reason: "UNRELATED_BUSINESS",
		label: "Cryptocurrency"
	},
	{
		regex: /\b(weather\s+(today|tomorrow|in)|temperature\s+outside|will\s+it\s+rain)\b/i,
		reason: "UNRELATED_BUSINESS",
		label: "Weather Report"
	},
	{
		regex: /^what\s+is\s+a\s+(savings\s+account|current\s+account|fixed\s+deposit|recurring\s+deposit|debit\s+card|loan|mortgage|banking)\??$/i,
		reason: "GENERIC_BANKING_CONCEPT",
		label: "Generic Banking Definition"
	},
	{
		regex: /^explain\s+(compound\s+interest|what\s+is\s+banking|how\s+do\s+banks\s+work)\??$/i,
		reason: "GENERIC_BANKING_CONCEPT",
		label: "Banking Tutorial"
	}
], b = [
	{
		...
		options: [
			{
				...
				prompt: "Top performing branches by deposit growth this quarter",
				description: "Deposits and loan growth by branch"
			},
			{
				label: "Employee Productivity",
				prompt: "Average transactions processed per employee by branch",
				description: "Staff throughput & performance scores"
			},
			{
				label: "Loan Portfolio Quality",
				prompt: "NPA ratio and overdue trends across all loan types",
				description: "Delinquency and recovery tracking"
			},
			{
				label: "Financial Performance",
				prompt: "Revenue and profitability summary by region",
				description: "Net margins and income distribution"
			}
		]
	},
	{
		...
		options: [
			{
				label: "Loan Growth",
				prompt: "Which branches had loan growth above 15% YoY?",
				description: "Advance portfolio expansion"
			},
			{
				label: "Deposit Growth",
				prompt: "Top branches by deposit growth this financial year",
				description: "Savings and current balance trends"
			},
			{
				label: "Transaction Growth",
				prompt: "Transaction volume growth month-over-month by channel",
				description: "Digital vs branch velocity"
			},
			{
				label: "Customer Acquisition",
				prompt: "New customer onboarding count by zone for the last 6 months",
				description: "Account opening momentum"
			}
		]
	},
	{
		...
		options: [
			{
				label: "Deposit Trends",
				prompt: "Deposit growth trends by branch over the last 4 quarters",
				description: "Balance movement analysis"
			},
			{
				label: "Loan Disbursements",
				prompt: "Quarterly loan disbursement trends for the past 2 financial years",
				description: "Credit sanction volume"
			},
			{
				label: "NPA Ratio Trends",
				prompt: "NPA ratio trends across branches for the past 4 quarters",
				description: "Credit asset quality evolution"
			},
			{
				label: "Transaction Volume",
				prompt: "Monthly transaction count and volume trends by channel",
				description: "Operational channel activity"
			}
		]
	},
	{
		...
		options: [
			{
				label: "Loan Portfolio Status",
				prompt: "Active, closed, and NPA loan breakdown by branch",
				description: "Credit status distribution"
			},
			{
				label: "Account Status",
				prompt: "Count of active, dormant, and frozen accounts by branch",
				description: "Deposit account health"
			},
			{
				label: "Customer KYC Status",
				prompt: "Breakdown of customers by risk category and KYC status",
				description: "Compliance segmentation"
			}
		]
	}
], x = /* @__PURE__ */ "customer.client.borrower.depositor.account.savings.current account.fixed deposit.balance.loan.advance.npa.disbursement.sanction.portfolio.interest rate.emi.transaction.txn.deposit.withdrawal.transfer.debit.credit.channel.branch.zone.region.state.territory.employee.staff.manager.headcount.salary.attrition.productivity.department.complaint.interaction.product".split("."), S = /* @__PURE__ */ "growth.decline.yoy.qoq.financial year.fy.ratio.percentage.pct.total.average.avg.sum.count.max.min.volume.turnover.highest.lowest.top.bottom.rank.ranking.compare.comparison.overdue.delinquent.provision.yield.margin.profitability.revenue".split("."), C = [
	"ranchi",
	"jharkhand",
	"dhanbad",
	"jamshedpur",
	...
	classify(e, t = []) {
		...
		for (let e of g) if (e.test(r)) return this.buildSecurityDecision(n, "Prompt injection attempt detected: Instruction override or system prompt extraction is prohibited.", "System integrity guardrail triggered");
		for (let e of _) if (e.test(r)) return this.buildSecurityDecision(n, "Security policy: System credentials, internal configuration, and secrets cannot be accessed.", "Credential protection guardrail triggered");
		for (let e of v) if (e.regex.test(r)) return this.buildUnsupportedDecision(n, e.entity, e.reason);
		for (let e of b) if (e.regex.test(r)) return this.buildAmbiguousDecision(n, e.clarificationPrompt, e.options);
		for (let e of y) if (e.regex.test(r)) return this.buildOutOfScopeDecision(n, e.reason, e.label);
		let i = [...t].reverse().find((e) => e.classification === "IN_SCOPE"), a = w.some((e) => e.test(r));
		if (i && (a || this.isShortRefinement(r))) return this.buildInScopeDecision(n, i.entities || ["inherited"], ["trend", "comparison"], i.domain || "BANK_ANALYTICS", "COMPLEX_ANALYTICS", .94, !0);
		let o = x.filter((e) => r.includes(e)), s = S.filter((e) => r.includes(e)), c = C.filter((e) => r.includes(e)), l = o.length + s.length + c.length;
		if (o.length > 0 || s.length > 0 && c.length > 0) {
			let e = this.inferIntent(s, r), t = this.inferDomain(o), i = Math.min(.99, .85 + l * .03);
			return this.buildInScopeDecision(n, o, s, t, e, i, !1);
		}
		return {
			allowed: !1,
			...
			reasons: ["The request does not match recognized banking database entities or analytical metrics.", "Request blocked before SQL generation to prevent unverified data access."],
	...
	isShortRefinement(e) {
		return e.split(/\s+/).length <= 5 && (C.some((t) => e.includes(t)) || /\b(last\s+year|last\s+quarter|this\s+quarter|yoy|qoq|highest|lowest)\b/i.test(e));
	}
	inferIntent(e, t) {
		return /exceeded.*while.*declined|while.*consecutive/i.test(t) ? "COMPLEX_ANALYTICS" : e.some((e) => [
			"yoy",
			"qoq",
			"trend",
			"trends",
			"growth",
			"decline"
		].includes(e)) ? "TREND" : e.some((e) => [
			"compare",
			"comparison",
			"versus",
			"vs"
		].includes(e)) ? "COMPARISON" : e.some((e) => [
			"highest",
			"lowest",
			"top",
			"bottom",
			"rank",
			"ranking"
		].includes(e)) ? "RANKING" : e.some((e) => [
			"total",
			"average",
			"avg",
			"sum",
			"count"
		].includes(e)) ? "AGGREGATION" : "RETRIEVAL";
	}
	inferDomain(e) {
		return e.some((e) => [
			"loan",
			...
			"sanction",
			"advance"
		].includes(e)) ? "LOAN_ANALYTICS" : e.some((e) => [
			"transaction",
			"txn",
			"deposit",
			"withdrawal"
		].includes(e)) ? "TRANSACTION_ANALYTICS" : e.some((e) => [
			"customer",
			"client",
			"depositor",
			"borrower"
		].includes(e)) ? "CUSTOMER_ANALYTICS" : e.some((e) => [
			"account",
			"savings",
			"current account",
			"balance"
		].includes(e)) ? "ACCOUNT_ANALYTICS" : e.some((e) => [
			"employee",
			"staff",
			"salary",
			"attrition",
			"productivity"
		].includes(e)) ? "WORKFORCE_ANALYTICS" : e.some((e) => [
			"branch",
			"zone",
			"region",
			"state"
		].includes(e)) ? "BRANCH_NETWORK" : "GENERAL_BANKING_DATA";
	}
	buildSecurityDecision(e, t, n) {
		return {
			allowed: !1,
			...
			reasons: [
				t,
				n,
				"Blocked immediately before query planning or semantic parsing."
			],
	...
	buildUnsupportedDecision(e, t, n) {
		return {
			allowed: !1,
	...
	buildAmbiguousDecision(e, t, n) {
		return {
			...
			message: t,
			reasons: ["The query is under-specified and references multiple potential banking metrics.", "Clarification requested to ensure precise analytical results."],
			clarificationPrompt: t,
			clarificationOptions: n,
			contract: {
				scope: "BANK_DATA",
				classification: "AMBIGUOUS",
				confidence: .9,
	...
	buildOutOfScopeDecision(e, t, n) {
		return {
			...
			message: "I'm designed to answer questions related to authorized banking data and analytics. Please ask a question about customers, accounts, transactions, loans, branches, employees, or other available bank data.",
			reasons: [
				`The request was identified as ${n} (${t}).`,
			...
			contract: {
				...
				classification: "OUT_OF_SCOPE",
				confidence: .96,
				outOfScopeSubcategory: t,
				entities: [],
				metrics: [],
				requires_database: !1,
				requires_sql: !1,
				reasons: [`Identified as ${n}`],
				suggestedQuery: "Which branches had the highest loan growth this financial year?"
			}
		};
	}
	buildInScopeDecision(e, t, n, r, i, a, o) {
		return {
			allowed: !0,
			...
			reasons: [
				`Target domain: ${r}`,
				`Analytical intent: ${i}`,
				`Recognized entities: ${t.join(", ") || "implicit"}`
			],
			contract: {
				scope: "BANK_DATA",
				classification: "IN_SCOPE",
				...
				requires_database: !0,
				requires_sql: !0,
				reasons: [`Matches ${r} with ${i} intent`]
			}
		};
	}
}(), E = class {
	...
	async processQuery(e, t) {
		...
		try {
			let t = performance.now();
			r("Checking query scope", "start", 1);
			let i = this.getConversationContext(e), l = i.history.map((e) => ({
				question: e.question,
				classification: "IN_SCOPE",
				timestamp: Date.now()
			})), u = T.classify(e.question, l), d = Math.round(performance.now() - t);
			if (!u.allowed) return r("Checking query scope", "error", 1, u.classification), o.push({
				name: "Query Scope Guardrail",
				status: "error",
				durationMs: d,
				details: `${u.classification} — ${u.headline}`
				...
				executionTimeMs: Math.round(performance.now() - a),
				rowCount: 0,
				repairAttempts: 0
			}), {
				requestId: n,
				...
				scopeBlocked: !0,
				error: u.message,
				errorType: u.classification === "AMBIGUOUS" ? "ambiguity" : "validation",
				clarificationOptions: u.clarificationOptions?.map((e) => ({
					label: e.label,
					description: e.description || e.prompt,
					value: e.prompt
				})),
				guardrail: {
					...
					headline: u.headline,
					reasons: u.reasons,
					suggestedQuery: u.suggestedQuery
				},
				debug: this.buildDebugMetadata(n, "", [], [], "", `blocked_${u.classification.toLowerCase()}`, Math.round(performance.now() - a), 0, 0, 0, o)
			};
			r("Checking query scope", "done", 1, `${u.classification} (${Math.round(u.confidence * 100)}%)`), o.push({
				name: "Query Scope Guardrail",
				status: "success",
				durationMs: d,
				details: `In scope: ${u.contract.domain || "Banking Data"}`
			});
			let f = performance.now();
			r("Understanding the question", "start", 2);
			let p = this.schemaRetriever.retrieve(e.question);
			if (r("Understanding the question", "done", 2, `${p.retrievedTableNames.length} tables`), o.push({
				name: "Schema & Semantic Retrieval",
				status: "success",
				durationMs: Math.round(performance.now() - f),
				details: `Retrieved ${p.retrievedTableNames.length} tables`
			}), p.hasAmbiguity) {
				let e = p.semanticResolution.ambiguousTerms[0], t = e.possibleMeanings.map((e) => ({
					label: e.label,
					description: e.description,
					value: e.label
				}));
			...
			let m = performance.now(), h;
			if (r("Planning the query", "start", 3), this.config.fastMode && !this.shouldPlan(e.question)) h = this.buildInlinePlan(e.question, p, i), r("Planning the query", "skipped", 3, "not needed for this question");
			else {
				h = await this.generateSQLPlan(e.question, p, i);
				let t = Math.round(performance.now() - m);
				s += t, o.push({
					name: "SQL Planning",
					status: "success",
					durationMs: t,
					details: `Intent: ${h.intent}`
				}), r("Planning the query", "done", 3, `${t} ms`);
			}
			let g = performance.now();
			r("Writing SQL", "start", 4);
			let _ = await this.generateSQL(h, p);
			r("Writing SQL", "done", 4, `${Math.round(performance.now() - g)} ms`);
			let v = Math.round(performance.now() - g);
			s += v, o.push({
				name: "SQL Generation",
				status: "success",
				durationMs: v,
				details: `Generated ${_.length} chars`
			});
			let y = performance.now();
			r("Checking safety", "start", 5);
			let b = this.guardian.validate(_);
			if (r("Checking safety", b.valid ? "done" : "error", 5, b.valid ? "passed guardrails" : "repairing"), o.push({
				name: "SQL Validation",
				status: b.valid ? "success" : "error",
				durationMs: Math.round(performance.now() - y),
				details: b.valid ? `Passed (${b.warnings.length} warnings)` : `Failed: ${b.errors.map((e) => e.message).join("; ")}`
			}), !b.valid) {
				let e = await this.repairSQL(_, b, p, h);
				if (c = e.attempts, s += e.llmLatencyMs, e.success) _ = e.sql, b = e.validation, o.push({
					name: "SQL Repair",
					status: "success",
					...
					sqlPlan: h,
					validationResult: b,
					error: `Generated SQL failed validation: ${b.errors.map((e) => e.message).join(". ")}`,
					errorType: "validation",
					debug: this.buildDebugMetadata(n, "", p.retrievedTableNames, this.getBusinessDefs(p), _, "validation_failed", 0, s, 0, c, o)
				};
			}
			let x = b.modifiedSql || _, S = performance.now();
			r("Querying the database", "start", 6);
			try {
				let t = await this.db.executeQuery(x, [], this.config.sqlTimeoutMs);
				r("Querying the database", "done", 6, `${t.rowCount} rows`);
				let i = Math.round(performance.now() - S);
				o.push({
					name: "SQL Execution",
					status: "success",
					durationMs: i,
				...
				let l = performance.now(), u = t.rowCount >= this.config.maxResultRows;
				r(t.rowCount === 0 ? "Explaining the empty result" : "Summarising", "start", 7);
				let d, f;
				if (t.rowCount === 0) {
					let t = await this.diagnoseEmptyResult(e.question, x, p);
					d = t.probes.length > 0 ? t.probes : void 0, f = {
						summary: t.summary,
						filters: h.filters
					};
				} else f = await this.generateSummary(e.question, h, t.rows, t.rowCount, u);
				let m = Math.round(performance.now() - l);
				s += m, o.push({
					name: "Result Summary",
					status: "success",
					durationMs: m
				}), r("Summarising", "done", 7), this.updateConversationContext(e, _, h, p.retrievedTableNames);
				let g = Math.round(performance.now() - a);
				return this.auditLogger.log({
					...
					userQuestion: e.question,
					model: this.llm.getConfig().model,
					retrievedTables: p.retrievedTableNames,
					retrievedBusinessRules: this.getBusinessDefs(p),
					generatedSql: _,
					validationResult: b.valid ? "passed" : "failed",
					...
					sqlPlan: h,
					validationResult: b,
					debug: this.buildDebugMetadata(n, h.intent, p.retrievedTableNames, this.getBusinessDefs(p), _, "success", g, s, t.rowCount, c, o)
				};
			} catch (t) {
				o.push({
					name: "SQL Execution",
				...
				let r = await this.repairSQLFromError(_, t.message, p, h);
				if (c += r.attempts, s += r.llmLatencyMs, r.success) {
					let t = performance.now(), i = await this.db.executeQuery(r.validation.modifiedSql || r.sql, [], this.config.sqlTimeoutMs);
					o.push({
						name: "SQL Repair + Re-execution",
						status: "success",
						durationMs: Math.round(performance.now() - t),
						details: `Fixed and got ${i.rowCount} rows`
					});
					let l = i.rowCount >= this.config.maxResultRows, u = await this.generateSummary(e.question, h, i.rows, i.rowCount, l);
					s += 500;
					let d = Math.round(performance.now() - a);
					return this.updateConversationContext(e, r.sql, h, p.retrievedTableNames), this.auditLogger.log({
						requestId: n,
						userId: e.userId || "anonymous",
						timestamp: (/* @__PURE__ */ new Date()).toISOString(),
						userQuestion: e.question,
						...
						truncated: l,
						summary: u.summary,
						filtersApplied: u.filters,
						executionTimeMs: d,
						sql: r.sql,
						sqlPlan: h,
						validationResult: r.validation,
						debug: this.buildDebugMetadata(n, h.intent, p.retrievedTableNames, this.getBusinessDefs(p), r.sql, "success_after_repair", d, s, i.rowCount, c, o)
					};
				}
				let i = Math.round(performance.now() - a);
				return {
					requestId: n,
					success: !1,
					sql: _,
					...
					validationResult: b,
					executionTimeMs: i,
					debug: this.buildDebugMetadata(n, h.intent, p.retrievedTableNames, this.getBusinessDefs(p), _, "execution_failed", i, s, 0, c, o)
				};
			}
		} catch (e) {
...
async function D(e) {
	...
	let f = new u(i, a), g = new m(i, {
		...
		dialect: "postgresql"
	}), _ = new h(e.auditDir), v = new E(r, f, g, t, _, {
...
//#endregion
export { A as healthCheck, D as initializeBackend };
