...
var SchemaConfigLoader = class {
	...
	*/
	load(schemaIntelligence, semanticLayer) {
		...
		const catalogPath = path.default.join(this.basePath, "schema_catalog.json");
		if (fs.default.existsSync(catalogPath)) {
			const catalog = JSON.parse(fs.default.readFileSync(catalogPath, "utf-8"));
		...
		const relPath = path.default.join(this.basePath, "semantic", "relationships.json");
		if (fs.default.existsSync(relPath)) {
			const data = JSON.parse(fs.default.readFileSync(relPath, "utf-8"));
		...
		const glossaryPath = path.default.join(this.basePath, "semantic", "business_glossary.json");
		if (fs.default.existsSync(glossaryPath)) {
			const data = JSON.parse(fs.default.readFileSync(glossaryPath, "utf-8"));
		...
		const rulesPath = path.default.join(this.basePath, "semantic", "business_rules.json");
		if (fs.default.existsSync(rulesPath)) {
			const data = JSON.parse(fs.default.readFileSync(rulesPath, "utf-8"));
		...
		const entitiesPath = path.default.join(this.basePath, "semantic", "entities.json");
		if (fs.default.existsSync(entitiesPath)) {
			const data = JSON.parse(fs.default.readFileSync(entitiesPath, "utf-8"));
		...
		const metricsPath = path.default.join(this.basePath, "semantic", "metrics.json");
		if (fs.default.existsSync(metricsPath)) {
			const data = JSON.parse(fs.default.readFileSync(metricsPath, "utf-8"));
...
var LLMProvider = class {
	...
	generateRequestId() {
		return `req_${(0, crypto.randomUUID)().replace(/-/g, "").substring(0, 16)}`;
	}
...
var AuditLogger = class {
	...
	inMemoryLog = [];
	constructor(logDir) {
		this.logDir = logDir || path.default.join(process.cwd(), "audit_logs");
		this.logFile = path.default.join(this.logDir, `audit_${this.getDateStamp()}.jsonl`);
		try {
			if (!fs.default.existsSync(this.logDir)) fs.default.mkdirSync(this.logDir, { recursive: true });
		} catch (error) {
	...
	*/
	log(entry) {
		...
		try {
			fs.default.appendFileSync(this.logFile, JSON.stringify(sanitized) + "\n", "utf-8");
		} catch (error) {
...
//#endregion
//#region src/lib/guardrail/classifier.ts
var SECURITY_INJECTION_PATTERNS = [
...
var SECURITY_CREDENTIAL_PATTERNS = [
...
var UNSUPPORTED_SCHEMA_PATTERNS = [
	{
...
var QueryGuardrail = class {
	...
	*/
	classify(question, history = []) {
		...
		for (const pattern of SECURITY_INJECTION_PATTERNS) if (pattern.test(q)) return this.buildSecurityDecision(raw, "Prompt injection attempt detected: Instruction override or system prompt extraction is prohibited.", "System integrity guardrail triggered");
		for (const pattern of SECURITY_CREDENTIAL_PATTERNS) if (pattern.test(q)) return this.buildSecurityDecision(raw, "Security policy: System credentials, internal configuration, and secrets cannot be accessed.", "Credential protection guardrail triggered");
		for (const unsup of UNSUPPORTED_SCHEMA_PATTERNS) if (unsup.regex.test(q)) return this.buildUnsupportedDecision(raw, unsup.entity, unsup.reason);
		for (const ambig of AMBIGUOUS_PATTERNS) if (ambig.regex.test(q)) return this.buildAmbiguousDecision(raw, ambig.clarificationPrompt, ambig.options);
		for (const oos of OUT_OF_SCOPE_PATTERNS) if (oos.regex.test(q)) return this.buildOutOfScopeDecision(raw, oos.reason, oos.label);
		...
		if (lastInScope && (isFollowUpPattern || this.isShortRefinement(q))) return this.buildInScopeDecision(raw, lastInScope.entities || ["inherited"], ["trend", "comparison"], lastInScope.domain || "BANK_ANALYTICS", "COMPLEX_ANALYTICS", .94, true);
		...
		if (matchedEntities.length > 0 || matchedMetrics.length > 0 && matchedGeos.length > 0) {
			...
			const confidence = Math.min(.99, .85 + totalSignals * .03);
			return this.buildInScopeDecision(raw, matchedEntities, matchedMetrics, domain, intent, confidence, false);
		}
		return {
			allowed: false,
	...
	isShortRefinement(q) {
		return q.split(/\s+/).length <= 5 && (BANKING_GEO_TERMS.some((g) => q.includes(g)) || /\b(last\s+year|last\s+quarter|this\s+quarter|yoy|qoq|highest|lowest)\b/i.test(q));
	}
	inferIntent(metrics, q) {
		...
		if (metrics.some((m) => [
			...
			"growth",
			"decline"
		].includes(m))) return "TREND";
		if (metrics.some((m) => [
			"compare",
			"comparison",
			"versus",
			"vs"
		].includes(m))) return "COMPARISON";
		if (metrics.some((m) => [
			"highest",
			"lowest",
			"top",
		...
		if (metrics.some((m) => [
			...
			"sum",
			"count"
		].includes(m))) return "AGGREGATION";
		return "RETRIEVAL";
	}
	inferDomain(entities) {
		if (entities.some((e) => [
			"loan",
			"npa",
			"disbursement",
			"sanction",
			"advance"
		].includes(e))) return "LOAN_ANALYTICS";
		if (entities.some((e) => [
			"transaction",
			"txn",
			"deposit",
			"withdrawal"
		].includes(e))) return "TRANSACTION_ANALYTICS";
		if (entities.some((e) => [
			"customer",
			"client",
			"depositor",
			"borrower"
		].includes(e))) return "CUSTOMER_ANALYTICS";
		if (entities.some((e) => [
			"account",
			"savings",
			"current account",
			"balance"
		].includes(e))) return "ACCOUNT_ANALYTICS";
		if (entities.some((e) => [
			"employee",
			"staff",
			"salary",
			"attrition",
			"productivity"
		].includes(e))) return "WORKFORCE_ANALYTICS";
		if (entities.some((e) => [
			"branch",
			"zone",
			"region",
			"state"
		].includes(e))) return "BRANCH_NETWORK";
		return "GENERAL_BANKING_DATA";
	}
	buildSecurityDecision(raw, reason, rule) {
		return {
			allowed: false,
	...
	buildUnsupportedDecision(raw, entity, reason) {
		return {
			allowed: false,
	...
	buildAmbiguousDecision(raw, prompt, options) {
		return {
			allowed: false,
	...
	buildOutOfScopeDecision(raw, reason, label) {
		return {
			allowed: false,
			classification: "OUT_OF_SCOPE",
			confidence: .96,
	...
	buildInScopeDecision(raw, entities, metrics, domain, intent, confidence, isFollowUp) {
		return {
			allowed: true,
...
var queryGuardrail = new QueryGuardrail();
//#endregion
...
var QueryOrchestrator = class {
	...
	*/
	async processQuery(request, onProgress) {
		const requestId = `req_${(0, crypto.randomUUID)().replace(/-/g, "").substring(0, 16)}`;
		const TOTAL_STAGES = 7;
		const emit = (stage, status, index, detail) => {
			try {
				onProgress?.({
					stage,
					status,
		...
		try {
			...
			const historyForGuardrail = ((request.sessionId ? this.conversations.get(request.sessionId) : void 0)?.history || []).map((h) => ({
				question: h.question,
				classification: "IN_SCOPE",
				timestamp: Date.now()
			}));
			...
			if (!guardDecision.allowed) {
				...
				return {
					...
					debug: this.buildDebugMetadata(requestId, "", [], [], "", `blocked_${guardDecision.classification.toLowerCase()}`, Math.round(performance.now() - pipelineStart), 0, 0, 0, stages)
				};
			}
			...
			emit("Checking safety", validation.valid ? "done" : "error", 5, validation.valid ? "passed guardrails" : "repairing");
			stages.push({
				name: "SQL Validation",
