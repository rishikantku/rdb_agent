...
var a = class {
	...
	async processQuery(e) {
		let t = `req_${i().replace(/-/g, "").substring(0, 16)}`, n = performance.now(), r = [], a = 0, o = 0;
		try {
			let i = performance.now(), s = this.schemaRetriever.retrieve(e.question);
			if (r.push({
				...
				return {
					...
					debug: this.buildDebugMetadata(t, "", [], [], "", "ambiguity", 0, 0, 0, 0, r)
				};
			}
			...
			if (r.push({
				...
				if (o = e.attempts, a += e.llmLatencyMs, e.success) p = e.sql, g = e.validation, r.push({
					name: "SQL Repair",
					status: "success",
					...
					debug: this.buildDebugMetadata(t, "", s.retrievedTableNames, this.getBusinessDefs(s), p, "validation_failed", 0, a, 0, o, r)
				};
			}
			let _ = g.modifiedSql || p, v = performance.now();
			try {
				let i = await this.db.executeQuery(_, [], this.config.sqlTimeoutMs), c = Math.round(performance.now() - v);
				r.push({
					name: "SQL Execution",
					status: "success",
					durationMs: c,
					details: `${i.rowCount} rows in ${i.executionTimeMs}ms`
				});
				let l = performance.now(), d = i.rowCount >= this.config.maxResultRows, f, m;
				if (i.rowCount === 0) {
				...
				} else m = await this.generateSummary(e.question, u, i.rows, i.rowCount, d);
				...
				return this.auditLogger.log({
					...
					sqlPlan: u,
					validationResult: g,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), p, "success", y, a, i.rowCount, o, r)
				};
			} catch (i) {
				r.push({
					name: "SQL Execution",
				...
				let c = await this.repairSQLFromError(p, i.message, s, u);
				if (o += c.attempts, a += c.llmLatencyMs, c.success) {
					let i = performance.now(), l = await this.db.executeQuery(c.validation.modifiedSql || c.sql, [], this.config.sqlTimeoutMs);
					r.push({
					...
					return this.updateConversationContext(e, c.sql, u, s.retrievedTableNames), this.auditLogger.log({
						...
						debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), c.sql, "success_after_repair", p, a, l.rowCount, o, r)
					};
				}
				...
				return {
					...
					success: !1,
					sql: p,
					error: `I was unable to generate a correct query for this request. The database returned: ${i.message}`,
					errorType: "execution",
					sqlPlan: u,
					validationResult: g,
					executionTimeMs: l,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), p, "execution_failed", l, a, 0, o, r)
				};
			}
		} catch (e) {
			let i = Math.round(performance.now() - n), s = e.message?.includes("LLM") ? "llm" : "system";
			return {
				requestId: t,
				success: !1,
				...
				errorType: s,
				executionTimeMs: i,
				debug: this.buildDebugMetadata(t, "", [], [], "", `error_${s}`, i, a, 0, o, r)
			};
		}
	}
