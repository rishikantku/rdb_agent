import { n as e, t } from "./esm-BeI3rgKT.js";
import n from "path";
...
var a = class {
	...
	async connect() {
		...
		this.config.ssl && (t.ssl = typeof this.config.ssl == "object" ? this.config.ssl : { rejectUnauthorized: !1 }), this.pool = new e(t), (await this.pool.connect()).release(), this.connected = !0;
	}
	async disconnect() {
	...
	load(e, t) {
		let r = 0, a = 0, o = 0, s = 0, c = 0, l = n.join(this.basePath, "schema_catalog.json");
		if (i.existsSync(l)) {
			let t = JSON.parse(i.readFileSync(l, "utf-8"));
			for (let [n, i] of Object.entries(t.tables)) {
				let a = [];
				for (let [e, t] of Object.entries(i.columns)) {
				...
				let o = {
					...
					tags: this.generateTags(n, i.description)
				};
				e.registerTable(o), r++;
			}
			if (t.views) for (let n of t.views) e.registerTable({
				name: n,
				type: "view",
				schema: t.schema || "public",
			...
			if (t.join_paths) for (let [n, r] of Object.entries(t.join_paths)) {
				let t = r.split(" → ").map((e) => e.trim());
				e.registerJoinPattern({
					name: n,
					description: `Join path: ${r}`,
					tables: t,
					joinClause: this.buildJoinClause(n, t),
					useCases: [n.replace(/_/g, " ")]
				});
			}
			console.log(`[ConfigLoader] Loaded ${r} tables, ${a} views from schema catalog`);
		}
		let u = n.join(this.basePath, "semantic", "relationships.json");
		if (i.existsSync(u)) {
			let t = JSON.parse(i.readFileSync(u, "utf-8")), n = t.relationships || t;
			for (let t of n) e.registerRelationship({
				fromTable: t.from_table,
				fromColumn: t.from_column,
				toTable: t.to_table,
				toColumn: t.to_column,
				type: t.type || "many-to-one",
				description: t.description
			}), c++;
			console.log(`[ConfigLoader] Loaded ${c} relationships`);
		}
		let d = n.join(this.basePath, "semantic", "business_glossary.json");
		if (i.existsSync(d)) {
			let e = JSON.parse(i.readFileSync(d, "utf-8")), n = e.glossary || e;
			for (let [e, r] of Object.entries(n)) {
				let n = this.glossaryEntryToBusinessTerm(e, r);
				t.registerTerm(n), o++;
			}
			console.log(`[ConfigLoader] Loaded ${o} business terms`);
		}
		let f = n.join(this.basePath, "semantic", "business_rules.json");
		if (i.existsSync(f)) {
			let e = JSON.parse(i.readFileSync(f, "utf-8")), n = e.rules || e;
			for (let e of n) t.registerGlobalRule(`[${e.id}] ${e.name}: ${e.description}`), s++;
			console.log(`[ConfigLoader] Loaded ${s} business rules`);
		}
		let p = n.join(this.basePath, "semantic", "entities.json");
		if (i.existsSync(p)) {
			let t = JSON.parse(i.readFileSync(p, "utf-8")), n = t.entities || t;
			for (let [t, r] of Object.entries(n)) if (r.join_paths) for (let [n, i] of Object.entries(r.join_paths)) e.registerJoinPattern({
				name: `${t}_${n}`,
				description: `Join ${t} to ${n}`,
		...
		let m = n.join(this.basePath, "semantic", "metrics.json");
		if (i.existsSync(m)) {
			let e = JSON.parse(i.readFileSync(m, "utf-8")), n = e.metrics || e;
			for (let [e, r] of Object.entries(n)) {
				let n = this.generateMetricAliases(e, r.name);
				t.registerTerm({
			...
			console.log(`[ConfigLoader] Loaded ${Object.keys(n).length} metric definitions`);
		}
		return {
			tableCount: r,
			viewCount: a,
	...
	generateRequestId() {
		return `req_${r().replace(/-/g, "").substring(0, 16)}`;
	}
	async withLatency(e) {
	...
	validate(e) {
		...
		let l = e;
		return this.hasResultLimit(r) || (l = this.applyResultLimit(e), n.push({
			code: "LIMIT_APPLIED",
	...
	inMemoryLog = [];
	constructor(e) {
		this.logDir = e || n.join(process.cwd(), "audit_logs"), this.logFile = n.join(this.logDir, `audit_${this.getDateStamp()}.jsonl`);
		try {
			i.existsSync(this.logDir) || i.mkdirSync(this.logDir, { recursive: !0 });
		} catch (e) {
	...
	log(e) {
		...
		try {
			i.appendFileSync(this.logFile, JSON.stringify(t) + "\n", "utf-8");
		} catch (e) {
	...
	async processQuery(e) {
		let t = `req_${r().replace(/-/g, "").substring(0, 16)}`, n = performance.now(), i = [], a = 0, o = 0;
		try {
			let r = performance.now(), s = this.schemaRetriever.retrieve(e.question);
			if (i.push({
				name: "Schema & Semantic Retrieval",
				status: "success",
				durationMs: Math.round(performance.now() - r),
				details: `Retrieved ${s.retrievedTableNames.length} tables`
			}), s.hasAmbiguity) {
				let e = s.semanticResolution.ambiguousTerms[0], n = e.possibleMeanings.map((e) => ({
					label: e.label,
				...
				return {
					...
					debug: this.buildDebugMetadata(t, "", [], [], "", "ambiguity", 0, 0, 0, 0, i)
				};
			}
			let c = performance.now(), l = this.getConversationContext(e), u = await this.generateSQLPlan(e.question, s, l), d = Math.round(performance.now() - c);
			a += d, i.push({
				name: "SQL Planning",
				status: "success",
				durationMs: d,
				details: `Intent: ${u.intent}`
			});
			let f = performance.now(), p = await this.generateSQL(u, s), m = Math.round(performance.now() - f);
			a += m, i.push({
				name: "SQL Generation",
				status: "success",
				durationMs: m,
			...
			let h = performance.now(), g = this.guardian.validate(p);
			if (i.push({
				name: "SQL Validation",
				...
				let e = await this.repairSQL(p, g, s, u);
				if (o = e.attempts, a += e.llmLatencyMs, e.success) p = e.sql, g = e.validation, i.push({
					name: "SQL Repair",
					status: "success",
					...
					debug: this.buildDebugMetadata(t, "", s.retrievedTableNames, this.getBusinessDefs(s), p, "validation_failed", 0, a, 0, o, i)
				};
			}
			let _ = g.modifiedSql || p, v = performance.now();
			try {
				let r = await this.db.executeQuery(_, [], this.config.sqlTimeoutMs), c = Math.round(performance.now() - v);
				i.push({
					name: "SQL Execution",
					status: "success",
					durationMs: c,
					details: `${r.rowCount} rows in ${r.executionTimeMs}ms`
				});
				let l = performance.now(), d = r.rowCount >= this.config.maxResultRows, f, m;
				if (r.rowCount === 0) {
					let t = await this.diagnoseEmptyResult(e.question, _, s);
					f = t.probes.length > 0 ? t.probes : void 0, m = {
						summary: t.summary,
						filters: u.filters
					};
				} else m = await this.generateSummary(e.question, u, r.rows, r.rowCount, d);
				let h = Math.round(performance.now() - l);
				a += h, i.push({
					name: "Result Summary",
					status: "success",
					durationMs: h
				}), this.updateConversationContext(e, p, u, s.retrievedTableNames);
				let y = Math.round(performance.now() - n);
				return this.auditLogger.log({
					...
					retrievedTables: s.retrievedTableNames,
					retrievedBusinessRules: this.getBusinessDefs(s),
					generatedSql: p,
					...
					fields: r.fields,
					truncated: d,
					emptyResultDiagnosis: f,
					summary: m.summary,
					filtersApplied: m.filters,
					executionTimeMs: y,
					sql: p,
					sqlPlan: u,
					validationResult: g,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), p, "success", y, a, r.rowCount, o, i)
				};
			} catch (r) {
				i.push({
					name: "SQL Execution",
				...
				let c = await this.repairSQLFromError(p, r.message, s, u);
				if (o += c.attempts, a += c.llmLatencyMs, c.success) {
					let r = performance.now(), l = await this.db.executeQuery(c.validation.modifiedSql || c.sql, [], this.config.sqlTimeoutMs);
					i.push({
					...
					let d = l.rowCount >= this.config.maxResultRows, f = await this.generateSummary(e.question, u, l.rows, l.rowCount, d);
					a += 500;
					let p = Math.round(performance.now() - n);
					return this.updateConversationContext(e, c.sql, u, s.retrievedTableNames), this.auditLogger.log({
						requestId: t,
						userId: e.userId || "anonymous",
						timestamp: (/* @__PURE__ */ new Date()).toISOString(),
						userQuestion: e.question,
						...
						sql: c.sql,
						sqlPlan: u,
						validationResult: c.validation,
						debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), c.sql, "success_after_repair", p, a, l.rowCount, o, i)
					};
				}
				let l = Math.round(performance.now() - n);
				return {
					requestId: t,
					success: !1,
					sql: p,
					error: `I was unable to generate a correct query for this request. The database returned: ${r.message}`,
					errorType: "execution",
					sqlPlan: u,
					validationResult: g,
					executionTimeMs: l,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), p, "execution_failed", l, a, 0, o, i)
				};
			}
		} catch (e) {
			let r = Math.round(performance.now() - n), s = e.message?.includes("LLM") ? "llm" : "system";
			return {
				requestId: t,
				success: !1,
				...
				errorType: s,
				executionTimeMs: r,
				debug: this.buildDebugMetadata(t, "", [], [], "", `error_${s}`, r, a, 0, o, i)
			};
		}
	}
	async getDataCoverage() {
		...
		try {
			...
			let t = e.rows.map((e) => `SELECT '${e.table_name}.${e.column_name}' AS col, MIN(${e.column_name})::text AS lo, MAX(${e.column_name})::text AS hi FROM ${e.table_name}`), n = (await this.db.executeQuery(t.join(" UNION ALL "), [], this.config.sqlTimeoutMs)).rows.filter((e) => e.lo && e.hi).map((e) => `  ${e.col}: ${String(e.lo).slice(0, 10)} to ${String(e.hi).slice(0, 10)}`);
...
Respond with ONLY the SQL query. No explanations, no markdown, no code blocks.`, a = (await this.llm.generate({
			systemPrompt: i,
