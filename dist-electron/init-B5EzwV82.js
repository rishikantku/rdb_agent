...
var a = class {
	...
	async processQuery(e) {
		...
		try {
			...
			let c = performance.now(), l = this.getConversationContext(e), u;
			if (this.config.fastMode && !this.shouldPlan(e.question)) u = this.buildInlinePlan(e.question, s, l);
			else {
				u = await this.generateSQLPlan(e.question, s, l);
				let t = Math.round(performance.now() - c);
				a += t, r.push({
					name: "SQL Planning",
					status: "success",
					durationMs: t,
			...
			let d = performance.now(), f = await this.generateSQL(u, s), p = Math.round(performance.now() - d);
			a += p, r.push({
				name: "SQL Generation",
				status: "success",
				durationMs: p,
				details: `Generated ${f.length} chars`
			});
			let m = performance.now(), h = this.guardian.validate(f);
			if (r.push({
				name: "SQL Validation",
				status: h.valid ? "success" : "error",
				durationMs: Math.round(performance.now() - m),
				details: h.valid ? `Passed (${h.warnings.length} warnings)` : `Failed: ${h.errors.map((e) => e.message).join("; ")}`
			}), !h.valid) {
				let e = await this.repairSQL(f, h, s, u);
				if (o = e.attempts, a += e.llmLatencyMs, e.success) f = e.sql, h = e.validation, r.push({
					name: "SQL Repair",
					status: "success",
					...
					sqlPlan: u,
					validationResult: h,
					error: `Generated SQL failed validation: ${h.errors.map((e) => e.message).join(". ")}`,
					errorType: "validation",
					debug: this.buildDebugMetadata(t, "", s.retrievedTableNames, this.getBusinessDefs(s), f, "validation_failed", 0, a, 0, o, r)
				};
			}
			let g = h.modifiedSql || f, _ = performance.now();
			try {
				let i = await this.db.executeQuery(g, [], this.config.sqlTimeoutMs), c = Math.round(performance.now() - _);
				r.push({
					name: "SQL Execution",
					status: "success",
					durationMs: c,
				...
				let l = performance.now(), d = i.rowCount >= this.config.maxResultRows, p, m;
				if (i.rowCount === 0) {
					let t = await this.diagnoseEmptyResult(e.question, g, s);
					p = t.probes.length > 0 ? t.probes : void 0, m = {
						summary: t.summary,
						filters: u.filters
					};
				} else m = await this.generateSummary(e.question, u, i.rows, i.rowCount, d);
				let v = Math.round(performance.now() - l);
				a += v, r.push({
					name: "Result Summary",
					status: "success",
					durationMs: v
				}), this.updateConversationContext(e, f, u, s.retrievedTableNames);
				...
				return this.auditLogger.log({
					...
					generatedSql: f,
					validationResult: h.valid ? "passed" : "failed",
					...
					sqlPlan: u,
					validationResult: h,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), f, "success", y, a, i.rowCount, o, r)
				};
			} catch (i) {
				r.push({
					name: "SQL Execution",
				...
				let c = await this.repairSQLFromError(f, i.message, s, u);
				...
				return {
					...
					validationResult: h,
					executionTimeMs: l,
					debug: this.buildDebugMetadata(t, u.intent, s.retrievedTableNames, this.getBusinessDefs(s), f, "execution_failed", l, a, 0, o, r)
				};
			}
		} catch (e) {
