...
const ConnectionModal: React.FC<ConnectionModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(5px)' }}>
      <div className="glass fade-in" style={{ width: '520px', padding: '2rem', position: 'relative', background: '#0a192f', border: '1px solid var(--border)' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'transparent', color: '#8892b0' }}>
          <X size={20} />
        </button>

        <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Database color="#64ffda" /> Create New Connection
        </h2>
        <p style={{ color: '#8892b0', fontSize: '0.9rem', marginBottom: '1.5rem' }}>Add a new database environment to your Command Center.</p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          <div>
            <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Connection Name</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Oracle Production / SQL Server" />
          </div>

          <div>
            <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Database Type</label>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
              {(['sqlite', 'postgres', 'mysql', 'mssql', 'oracle'] as const).map(t => (
                <button
                  ...
                  style={{
                    padding: '0.5rem',
                    background: type === t ? 'rgba(100, 255, 218, 0.1)' : 'transparent',
                    border: `1px solid ${type === t ? 'var(--accent)' : 'var(--border)'}`,
                    color: type === t ? 'var(--accent)' : '#8892b0',
          ...
          {type === 'sqlite' ? (
            <div>
              <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Database File Path</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div style={{ gridColumn: 'span 2' }}>
                <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Host / Endpoint</label>
                <input value={details.host} onChange={e => setDetails({ ...details, host: e.target.value })} />
              </div>
              <div>
                <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Port</label>
                <input type="number" value={details.port} onChange={e => setDetails({ ...details, port: parseInt(e.target.value) })} />
              </div>
              <div>
                <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Database / SID</label>
                <input value={details.database} onChange={e => setDetails({ ...details, database: e.target.value })} />
              </div>
              <div>
                <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>User</label>
                <input value={details.user} onChange={e => setDetails({ ...details, user: e.target.value })} />
              </div>
              <div>
                <label style={{ fontSize: '0.8rem', color: '#64ffda', display: 'block', marginBottom: '0.4rem' }}>Password</label>
          ...
          {testResult && (
            <div style={{ 
              ...
              borderRadius: '6px', border: `1px solid ${testResult.success ? '#10b981' : '#ef4444'}`
            }}>
